...
# Taken from https://github.com/qrpike/Web-Font-Load/blob/master/install.sh, with licence being MIT at the time
# OS detection
osdetect=$(uname)
file_path="unknown"
...
curl -L https://github.com/google/fonts/tarball/6df94760ab91af2bcf38a4c8219a302d929b11d7 -o /tmp/master.tar.gz
echo "Extracting the fonts..."
mkdir -p /tmp/goog-fonts/fonts
tar -zxf /tmp/master.tar.gz -C /tmp/goog-fonts/fonts
find /tmp/goog-fonts/fonts/ -type f -name "*.ttf" -exec cp {} $file_path \;
fc-cache -f
